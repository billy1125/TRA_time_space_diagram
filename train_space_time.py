...
time_loc = Globals.TimeLocation         # 時間空間基本資料
...
class SpaceTime:           
    ...
    def CalculateSpaceTime(self, _train):
        ...
        line = _train['Line']                    # 西部幹線預計歷經：山線1、海線2、成追線3，東部幹線則為0
        over_night_stn = _train['OverNightStn']  # 跨午夜車站
        line_dir = _train['LineDir']             # 順行1、逆行2
        
        # 查詢表定台鐵時刻表所有「停靠」車站，可查詢特定車次
        dict_start_end_station = self._find_train_stations(_train)

        # 查詢特定車次所有「停靠與通過」車站
        list_passing_stations = self._find_passing_stations(dict_start_end_station, line, line_dir)

        # 推算所有通過車站的時間與位置
        list_train_time_space = self._estimate_time_space(dict_start_end_station, list_passing_stations)
    ...
    # 找出每一個車次的表定經過車站
    def _find_train_stations(self, train_no):
    ...
    # 推算所有通過車站的時間
    def _estimate_time_space(self, dict_start_end_station, list_passing_stations):
        global time_loc
